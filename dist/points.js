...
export function getCube(n, origin = new THREE.Vector3(0, 0, 0), scale = new THREE.Vector3(1, 1, 1)) {
    let colors = [];
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            for (let k = 0; k < n; k++) {
                const indexOffset = new THREE.Vector3(0, 0, 0
                // 0.01*i*j*k,
                // 0.01*i*j*k,
                // 0.01*i*j*k
                );
...
export function getGeometryPoints(geometry, origin = new THREE.Vector3(0, 0, 0)) {
    ...
    for (let i = 0; i < geoPosArr.length; i += 3) {
        ...
        color.setRGB(1 - i / geoPosArr.length, i / geoPosArr.length, 1, THREE.SRGBColorSpace);
        colors.push(color.r, color.g, color.b);
    }
    return { colors: colors, posArray: posArray };
}
export function getBallPoints(shellNum, shellDiff, origin = new THREE.Vector3(0, 0, 0)) {
...
export function getRingPoints(n, radius, origin = new THREE.Vector3(0, 0, 0)) {
    let posArray = [], colors = [];
...
export function getPointMeshArr(posArray, pixelSize = 0.7) {
