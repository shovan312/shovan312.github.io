...
let tempo = 50, mpb = 0, songChords = [], melody = [], mainGrid, gridItems, songLength = 0, key = 0, songName = "Moon";
function initSong(tmp = 50, songName = "Moon", key = 0) {
    tempo = tmp;
    mpb = 60 * 1000 / tempo; //1 bar @ 100bpm
    let song = getSong(songName, key);
    songChords = song.chordsArr;
    let songSections = song.rhythmSections;
...
function animateWrapper() {
    function animate() {
        if (!isAnimating) {
            ...
            gridItems[0].style.backgroundColor = "#e74c3c";
        ...
        if (!isPlaying) {
            ...
            for (let i = 0; i < gridItems.length; i++) {
                ...
                for (let j = 0; j < songChords[i + songLength * Math.floor(index / songLength)].length; j++) {
                    if (!songChords[i + songLength * Math.floor(index / songLength)][j].isDiatonic)
                        isAnyChordNotDiatonic = true;
                }
                isAnyChordNotDiatonic ? gridItems[i].style.backgroundColor = "#103166" : gridItems[i].style.backgroundColor = "#3498db";
                let barString = "";
...
document.getElementById("ddSongName").addEventListener("change", () => {
    const dropdown = document.getElementById("ddSongName");
